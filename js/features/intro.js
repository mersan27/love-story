 ...
 * Animated intro
 * ─────────────────────────────────────
 * Plays a short "unwrapping" sequence after the proposal scene wraps
 * up and before the landing page reveals itself. Purely cinematic —
 * no input required, with an optional skip for repeat visitors.
 *
 * This file doesn't know the gate or proposal exist. It just listens
 * for "proposal:yes" and, when it's done, emits "intro:complete" —
...
export function initIntro() {
  ...
  bus.on('proposal:yes', (detail) => {
    /* Returning visitor this session — the gate and proposal already
       skipped themselves, so skip this cinematic too and go straight in. */
    if (detail?.instant) {
